...
%% --- 2. RUN WITH PROPERLY CORRECTED STRETCH ---
...
for k = 1:NL
    ...
    % Correct material property calculation (same as FG_GOEAM.m)
...
Lambda0_s_proper = lamda0 * a^2 / (ECu * h^3);
Lambda1_s_proper = lamda1 * a^2 / (ECu * h^3);
Lambda2_s_proper = lamda2 * a^2 / (ECu * h^3);

fprintf('PROPER_STRETCH_START\n');
fprintf('No-Stretch:        Lambda0=%.6f, Lambda1=%.6f, Lambda2=%.6f\n', Lambda0_ns, Lambda1_ns, Lambda2_ns);
fprintf('Proper-Stretch:    Lambda0=%.6f, Lambda1=%.6f, Lambda2=%.6f\n', Lambda0_s_proper, Lambda1_s_proper, Lambda2_s_proper);
fprintf('PROPER_STRETCH_END\n');
