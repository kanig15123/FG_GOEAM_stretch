...
%% --- RUN WITH CORRECTED STRETCH ---
% Let's load the properties but overwrite the integration variables with corrected symmetry
...
%% Manual execution of FG_GOEAM_stretch with fixes:
T0 = 300;
DeltaT = T - T0;
ECu = 65.79e9; NuCu = 0.387; AlphaCu = 16.51e-6; RoCu = 8800; RoGr = 1800;
...
for k = 1:NL
    ...
    Ec_Fin = ECu * fE; Nu_Fin = NuCu * fv; Alpha_Fin = AlphaCu * fAlpha;
    if Nu_Fin > 0.49; Nu_Fin = 0.49; end
    if Nu_Fin < -0.99; Nu_Fin = -0.99; end 

    f_fun = @(z) z - c1*z.^3;
    g_fun = @(z) 1 - 3*c1*z.^2;
    
    constQ = 1 - Nu_Fin^2;
    Q11_val = Ec_Fin / constQ;
    Q12_val = Ec_Fin * Nu_Fin / constQ;
    Q66_val = Ec_Fin / (2*(1+Nu_Fin));
    Q44_val = Q66_val; 
    
    constC = (1 + Nu_Fin) * (1 - 2*Nu_Fin);
    C11_3D_val = Ec_Fin * (1 - Nu_Fin) / constC;
    C12_3D_val = Ec_Fin * Nu_Fin / constC;
    C33_3D_val = C11_3D_val; 
    ...
    beta_val = (Q11_val + Q12_val) * Alpha_Fin * DeltaT;
    ...
    Beta_z_val = (2*C12_3D_val + C33_3D_val) * Alpha_Fin * DeltaT;
    ...
    a21=integral(Q12a, z_bot, z_top, 'ArrayValued', true); % CORRECTED: Q12a instead of Q11a
    a22=integral(Q11a, z_bot, z_top, 'ArrayValued', true); % CORRECTED: Q11a instead of Q11a is fine since Q11=Q22
    ...
    b21=integral(Q12b, z_bot, z_top, 'ArrayValued', true); % CORRECTED: Q12b instead of Q11b
    ...
    d21=integral(Q12d, z_bot, z_top, 'ArrayValued', true); % CORRECTED: Q12d instead of Q11d
    ...
    e21=integral(Q12e, z_bot, z_top, 'ArrayValued', true); % CORRECTED: Q12e instead of Q11e
    ...
    f21=integral(Q12f, z_bot, z_top, 'ArrayValued', true); % CORRECTED: Q12f instead of Q11f
    ...
    h21=integral(Q12h, z_bot, z_top, 'ArrayValued', true); % CORRECTED: Q12h instead of Q11h
...
Lambda0_s_corr = lamda0 * a^2 / (ECu * h^3);
Lambda1_s_corr = lamda1 * a^2 / (ECu * h^3);
Lambda2_s_corr = lamda2 * a^2 / (ECu * h^3);

%% --- PRINT CORRECTED RESULTS ---
fprintf('CORRECTED_RESULT_START\n');
fprintf('No-Stretch:         Lambda0=%.6f, Lambda1=%.6f, Lambda2=%.6f\n', Lambda0_ns, Lambda1_ns, Lambda2_ns);
fprintf('Stretch (Uncorr):   Lambda0=%.6f, Lambda1=%.6f, Lambda2=%.6f\n', Lambda0_s, Lambda1_s, Lambda2_s);
fprintf('Stretch (Corrected):Lambda0=%.6f, Lambda1=%.6f, Lambda2=%.6f\n', Lambda0_s_corr, Lambda1_s_corr, Lambda2_s_corr);
fprintf('CORRECTED_RESULT_END\n');

%% --- SAVE CORRECTED PLOT ---
num_points = 100; 
Wn = linspace(0, 1, num_points);
Lambda_n_ns = Lambda0_ns + Lambda1_ns .* Wn + Lambda2_ns .* (Wn.^2);
Lambda_n_s_uncorr = Lambda0_s + Lambda1_s .* Wn + Lambda2_s .* (Wn.^2);
Lambda_n_s_corr = Lambda0_s_corr + Lambda1_s_corr .* Wn + Lambda2_s_corr .* (Wn.^2);

fig = figure('Visible', 'off');
plot(Wn, Lambda_n_ns, 'k--', 'LineWidth', 2.5); hold on;
plot(Wn, Lambda_n_s_uncorr, 'b:', 'LineWidth', 2.5);
plot(Wn, Lambda_n_s_corr, 'r-', 'LineWidth', 2.5);
grid on; box on;
set(gca, 'LineWidth', 1.5, 'FontSize', 14);
xlabel('\overline{W}');
ylabel('\overline{\lambda}');
legend('Standard HSDT (No Stretch)', 'Quasi-3D (With Typo Stretch)', 'Quasi-3D (Corrected Stretch)', 'Location', 'Best');
title('Postbuckling Load-Deflection Curve Comparison (With Corrections)');
saveas(fig, 'postbuckling_comparison_corrected.png');
