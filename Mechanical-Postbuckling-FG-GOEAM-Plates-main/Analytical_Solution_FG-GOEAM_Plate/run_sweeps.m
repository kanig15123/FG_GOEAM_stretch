...
% Default parameters for Table 3 & 4:
% a/h = 10, b/a = 1, NL = 10, no foundation (Ku=Kl=Ks=0)
h = 0.005; 
var1 = 10; % a/h
var2 = 1.0; % b/a
a = var1*h;
...
% Let's sweep temperatures for Table 3 (X-WGr, WGr = 2.5 wt%, HGr = 80%, Biaxial and Uniaxial)
T_range = [300, 320, 340, 360, 380, 400];
WGr = 2.5/100;
HGr = 80/100;

fprintf('========================================================================\n');
fprintf('TABLE 3 SWEEP (X-WGr, WGr=2.5 wt%%, HGr=80%%, a/h=10, b/a=1)\n');
fprintf('========================================================================\n');
fprintf('T(K)   LoadCase   No-Stretch   Stretch       Diff        %% Change\n');
fprintf('------------------------------------------------------------------------\n');
...
for T = T_range
    for load_case = 1:2
        ...
        % 1. No-Stretch
        % We need to use the distribution of X-WGr:
        % Let's set GOri_distribution to X-WGr pattern.
        % For X-WGr, distribution is:
        % VGr = 2 * VGrs * (2 * abs(z) / h);
        % Let's calculate the no-stretch value:
        % To avoid calling external scripts which might overwrite variables, let's write a small solver function.
        
        [Ncr_ns, Ncr_s] = solve_plate_Ncr(h, a, b, m, n, c1, c2, T, HGr, WGr, 2, Chi1, Chi2); % pattern=2 for X-WGr
        
        diff = Ncr_ns - Ncr_s;
        pct = (diff / Ncr_ns) * 100;
        fprintf('%d    %s   %.6f     %.6f     %.6f    %.4f%%\n', T, label, Ncr_ns, Ncr_s, diff, pct);
    end
end
